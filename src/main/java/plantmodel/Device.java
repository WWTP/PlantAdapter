...
public abstract class Device {
	...
	public IEndpointInterface[] getEndpointInterfaces() {
		List<IEndpointInterface> eis = new ArrayList<IEndpointInterface>();
		for (Endpoint e : this.getEndpoints()) {
			eis.addAll(Arrays.asList(e.getInterfaces()));
		}
		return eis.toArray(new AnalogueEndpointInterface[0]);
	}
	
	public IEndpointInterface[] getEndpointLogicalInterfaces() {
		List<IEndpointInterface> eis = new ArrayList<IEndpointInterface>();
		for (Endpoint e : this.getEndpoints()) {
			eis.addAll(Arrays.asList(e.getLogicalInterfaces()));
		}
		return eis.toArray(new IEndpointInterface[0]);
	}
	
	public IEndpointInterface[] getEndpointPhysicalInterfaces() {
		List<IEndpointInterface> eis = new ArrayList<IEndpointInterface>();
		for (Endpoint e : this.getEndpoints()) {
	...
	public IEndpointInterface[] getEndpointNativeInterfaces(Endpoint e) {
		return e.getPhysicalInterfaces();
	}
	
	public IEndpointInterface[] getEndpointNativeInterfaces(Endpoint e, Class<? extends Quantity> q) {
		return e.getPhysicalInterfacesForQuantity(q);
	}
	
	public IEndpointInterface getEndpointPreferredInterface(Endpoint e) {
		return e.getPreferredInterface();
	}
	
	public void setEndpointPreferredInterface(Endpoint e, IEndpointInterface ei) {
