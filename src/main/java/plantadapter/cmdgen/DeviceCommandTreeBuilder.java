...
class DeviceCommandTreeBuilder {
	...
	private class DeviceCommandTreeImpl implements IDeviceCommandTree{
		...
		private class MaskBuilder implements TreeNavigator<DeviceCommand> {
			...
			public void preorderVisit(TreeNode<DeviceCommand> node) {
				if (node.getChildren().size() > 0) { 
					// Aggiungo allo Stack prima di navigare i nodi figli, ma solo se non è già presente una maschera per la stessa istanza di Device
					// (o se lo Stack è vuoto)
					try  {
				...
				{
					// Se il comando è diretto ad un endpoint slave (e.g. porta di controllo di un dispositivo
					// controllato) allora devo anche aggiungere una maschera relativa a quel dispositivo (solamente 
					// se esso può aggiungere formato all'input, ossia se ah un'interfaccia DataAmount).
