	...
	public final SingleResult[] parse(IInputMask mask, IDeviceIOData input) throws MaskFormatException {
		...
		for (IInputMask subMask : mask.asMask()) {
			if (subMask.isCommand()) { 
				...
				// TODO Convenzione temporanea: se l risultato è null significa che il parser per qualche motivo ha deciso di ignorarlo...
				if (res != null) results.add(res);
				else {
					// TODO Necessaria add seguita da remove perché altrimenti al momento della toArray() non carica la classe...
